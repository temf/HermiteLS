...
class OptimResults(object):
    def __init__(self, xmin, fmin, gradmin, hessmin, nf, nx, nruns, exit_flag, exit_msg, n_iter):
        self.x = xmin
        self.f = fmin
    ...
    def __str__(self):
        ...
        output = "****** PyBOBYQA - Hermite least squares Results ******\n"
...
def solve_main(version, objfun, known_derivs, x0, args, xl, xu, npt, rhobeg, rhoend, maxfun, nruns_so_far, nf_so_far, nx_so_far, nsamples, params,
               diagnostic_info, scaling_changes, f0_avg_old=None, f0_nsamples_old=None, derivs0_old = None, do_logging=True, print_progress=False):
    ...
    if f0_avg_old is None:
        ...
        nf = nf_so_far + 1
        nx = nx_so_far + 1
        f0, derivs0 = eval_objective(objfun, remove_scaling(x0, scaling_changes), known_derivs, args, eval_num=nf, pt_num=nx,
        ...
        #print('f_list',f_list,'n samples',number_of_samples)
        f_list[0] = f0
        num_samples_run = 1
        exit_info = None
        ...
        for i in range(1, number_of_samples):  # skip first eval - already did this
            ...
            f_list[i] = eval_objective(objfun, remove_scaling(x0, scaling_changes), known_derivs, args, eval_num=nf, pt_num=nx,
                                                full_x_thresh=params("logging.n_to_print_whole_x_vector"),
                                                check_for_overflow=params("general.check_objfun_for_overflow"),
                                       verbose=do_logging, return_all = False)
            num_samples_run += 1
    ...
    control = Controller(version, objfun, known_derivs, x0, args, f0_avg, derivs0, num_samples_run, xl, xu, npt, rhobeg, rhoend, nf, nx, maxfun, params, scaling_changes, do_logging=do_logging)
    ...
    if exit_info is not None:
        # check if exit before first run?!
        x, f, derivsmin, gradmin, hessmin, nsamples = control.model.get_final_results()
        return x, f, derivs, None, None, nsamples, control.nf, control.nx, nruns_so_far + 1, exit_info, diagnostic_info
        ...
        if params("noise.quit_on_noise_level") and control.all_values_within_noise_level(params):
            if params("restarts.use_restarts") and params("restarts.use_soft_restarts"):
                ...
                exit_info = control.soft_restart(number_of_samples, nruns_so_far, params,
                                     x_in_abs_coords_to_save=None, f_to_save=None, nsamples_to_save=None, derivs_to_save=None)
                if exit_info is not None:
                    nruns_so_far += 1
                    break  # quit
                current_iter = -1
        ...
        if dnorm < params("general.safety_step_thresh") * control.rho:
                ...
                if exit_info is not None:
                    if exit_info.able_to_do_restart() and params("restarts.use_restarts") and params(
                        ...
                        exit_info = control.soft_restart(number_of_samples, nruns_so_far, params,
                                                         ...
                                                         nsamples_to_save=None, derivs_to_save=None)
                        if exit_info is not None:
                            nruns_so_far += 1
                            break  # quit
                        current_iter = -1
            ...
            if control.rho > rhoend:
                ...
                if params("restarts.use_restarts") and params("restarts.use_soft_restarts"):
                    ...
                    number_of_samples = max(nsamples(control.delta, control.rho, current_iter, nruns_so_far), 1)
                    f_list, derivs, num_samples_run, exit_info = control.evaluate_objective(x, number_of_samples,
                                                                                               params, return_all = True)
                    
                    if num_samples_run > 0:
                            control.model.save_point(x, np.mean(f_list[:num_samples_run]), num_samples_run, derivs, x_in_abs_coords=True)
            ...
            x = control.model.as_absolute_coordinates(xnew)
            number_of_samples = max(nsamples(control.delta, control.rho, current_iter, nruns_so_far), 1)
            f_list, derivs, num_samples_run, exit_info = control.evaluate_objective(x, number_of_samples, params, return_all = True)
            if exit_info is not None:
                if num_samples_run > 0:
                    control.model.save_point(x, np.mean(f_list[:num_samples_run]), num_samples_run, derivs, x_in_abs_coords=True)
                nruns_so_far += 1
            ...
            control.model.change_point(knew, xnew, f_list[0], derivs)  # expect step, not absolute x
    ...
    #print('number of iteraions:', current_iter)    
    
    
    return x, f, derivs, gradmin, hessmin, nsamples, control.nf, control.nx, nruns_so_far, exit_info, diagnostic_info, current_iter


def solve(version, objfun, known_derivs, x0, args=(), bounds=None, npt=None, rhobeg=None, rhoend=1e-8, maxfun=None, nsamples=None, user_params=None,
    ...
    if maxfun is None:
        ...
        maxfun = np.int(maxfun)
    if nsamples is None:
        nsamples = lambda delta, rho, iter, nruns: 1  # no averaging

    # Set parameters
    params = ParameterList(np.int(n), np.int(npt), np.int(maxfun), objfun_has_noise=objfun_has_noise, seek_global_minimum=seek_global_minimum)
    ...
    nf = 0
    nx = 0
    xmin, fmin, derivsmin, gradmin, hessmin, nsamples_min, nf, nx, nruns, exit_info, diagnostic_info, n_iter = \
        solve_main(version, objfun, known_derivs, x0, args, xl, xu, npt, rhobeg, rhoend, maxfun, nruns, nf, nx, nsamples, params,
    ...
    while params("restarts.use_restarts") and not params("restarts.use_soft_restarts") and nf < maxfun and \
        ...
        if params("restarts.hard.use_old_fk"):
            xmin2, fmin2, derivsmin2, gradmin2, hessmin2, nsamples2, nf, nx, nruns, exit_info, diagnostic_info, n_iter = \
                solve_main(version, objfun, known_derivs, xmin, args, xl, xu, npt, _rhobeg, _rhoend, maxfun, nruns, nf, nx, nsamples, params,
                            diagnostic_info, scaling_changes, f0_avg_old=fmin, f0_nsamples_old=nsamples_min, derivs0_old = derivsmin, do_logging=do_logging, print_progress=print_progress)
        else:
            xmin2, fmin2, derivsmin2, gradmin2, hessmin2, nsamples2, nf, nx, nruns, exit_info, diagnostic_info, n_iter = \
                solve_main(version, objfun, known_derivs, xmin, args, xl, xu, npt, _rhobeg, _rhoend, maxfun, nruns, nf, nx, nsamples, params,
                           diagnostic_info, scaling_changes, do_logging=do_logging, print_progress=print_progress)
        ...
        if fmin2 < fmin or np.isnan(fmin):
            ...
            last_successful_run = nruns
            (xmin, fmin, nsamples_min, derivsmin) = (xmin2, fmin2, nsamples2, derivsmin2)
    ...
    results = OptimResults(remove_scaling(xmin, scaling_changes), fmin, gradmin, hessmin, nf, nx, nruns, exit_flag, exit_msg, n_iter)
    if params("logging.save_diagnostic_info"):
