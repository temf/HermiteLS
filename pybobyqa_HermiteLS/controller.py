...
class Controller(object):
    def __init__(self, version, objfun, known_derivs, x0, args, f0, derivs0, f0_nsamples, xl, xu, npt, rhobeg, rhoend, nf, nx, maxfun, params, scaling_changes, do_logging=True):
        self.objfun = objfun
        self.maxfun = maxfun
        self.args = args
        self.model = Model(version, known_derivs, npt, x0, f0, derivs0, xl, xu, f0_nsamples, abs_tol=params("model.abs_tol"),
    ...
    def initialise_coordinate_directions(self, number_of_samples, num_directions, params):
        ...
        for k in range(1, num_directions + 1):
            ...
            f_list, derivs, num_samples_run, exit_info = self.evaluate_objective(x, number_of_samples, params, return_all = True)

            # Handle exit conditions (f < min obj value or maxfun reached)
            if exit_info is not None:
                if num_samples_run > 0:
                    self.model.save_point(x, np.mean(f_list[:num_samples_run]), num_samples_run, derivs,
                                          x_in_abs_coords=True)
                return exit_info  # return & quit

            # Otherwise, add new results (increments model.npt_so_far)
            self.model.change_point(k, x - self.model.xbase, f_list[0], derivs)  # expect step, not absolute x
    ...
    def initialise_random_directions(self, number_of_samples, num_directions, params):
        ...
        if params("init.run_in_parallel"):
            ...
            for ndirns in range(num_directions):
                ...
                eval_obj_results.append(self.evaluate_objective(x, number_of_samples, params, return_all = True))

            for ndirns in range(num_directions):
                new_point = xopt + dirns[ndirns, :]  # alway base move around best value so far
                x = self.model.as_absolute_coordinates(new_point)
                f_list, derivs, num_samples_run, exit_info = eval_obj_results[ndirns]
                # Handle exit conditions (f < min obj value or maxfun reached)
                if exit_info is not None:
                    if num_samples_run > 0:
                        self.model.save_point(x, np.mean(f_list[:num_samples_run]), num_samples_run, derivs,
    ...
    def geometry_step(self, knew, adelt, number_of_samples, params):
        ...
        if exit_info is not None:
            if num_samples_run > 0:
                self.model.save_point(x, np.mean(f_list[:num_samples_run]), num_samples_run, derivs, x_in_abs_coords=True)
            return exit_info  # didn't fix geometry - return & quit

        # Otherwise, add new results
        self.model.change_point(knew, xnew, f_list[0], derivs)  # expect step, not absolute x
        for i in range(1, num_samples_run):
    ...
    def evaluate_objective(self, x, number_of_samples, params, return_all = False):
        ...
        for i in range(number_of_samples):
            ...
            if number_of_samples == 1:
                f_list[i], derivs = eval_objective(self.objfun, remove_scaling(x, self.scaling_changes), self.model.known_derivs, self.args, eval_num=self.nf, pt_num=self.nx,
                                            full_x_thresh=params("logging.n_to_print_whole_x_vector"),
                                            check_for_overflow=params("general.check_objfun_for_overflow"),
                                       verbose=self.do_logging, return_all = True)
                num_samples_run += 1
            else:
                f_list[i] = eval_objective(self.objfun, remove_scaling(x, self.scaling_changes), self.model.known_derivs, self.args, eval_num=self.nf, pt_num=self.nx,
                                            full_x_thresh=params("logging.n_to_print_whole_x_vector"),
                                            check_for_overflow=params("general.check_objfun_for_overflow"),
                                       verbose=self.do_logging, return_all = False)
                num_samples_run += 1
                #print('ERROR: derivative calculation not working for average sampling - please modify!!!') #!!!!!!ToDo
                #break
        ...
        if return_all:
            return f_list, derivs, num_samples_run, exit_info
        else:
    ...
    def terminate_from_slow_iterations(self, current_iter, params):
        if len(self.last_iters_step_taken) <= params("slow.history_for_slow"):
            ...
            this_iter_slow = (self.last_fopts_step_taken[0] - self.model.fopt()) / \
                         np.float(params("slow.history_for_slow")) < params("slow.thresh_for_slow")
    ...
    def soft_restart(self, number_of_samples, nruns_so_far, params, x_in_abs_coords_to_save=None, f_to_save=None,
        ...
        if x_in_abs_coords_to_save is not None:
            ...
            self.model.save_point(x_in_abs_coords_to_save, f_to_save, nsamples_to_save, derivs_to_save, x_in_abs_coords=True)
        self.model.save_point(self.model.xopt(abs_coordinates=True), self.model.fopt(),
                              self.model.nsamples[self.model.kopt], self.model.derivsopt(), x_in_abs_coords=True)
