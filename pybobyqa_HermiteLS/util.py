...
def eval_objective_Cartis(objfun, x, args=(), verbose=True, eval_num=0, pt_num=0, full_x_thresh=6, check_for_overflow=True):
    # Evaluate objective function for original PyBOBYQA (by Cartis et al.)
...
def eval_objective(objfun, x, known_derivs, args=(), verbose=True, eval_num=0, pt_num=0, full_x_thresh=6, check_for_overflow=True, return_all = True):
    # Evaluate objective function for Hermite least squares
    if return_all == False:
        if check_for_overflow:
            try:
                f = objfun(x, known_derivs, return_all, *args)
            except OverflowError:
                f = sys.float_info.max
        else:
            f = objfun(x, known_derivs, return_all, *args)
        ...
        if check_for_overflow:
            try:
                f, derivs_all = objfun(x, known_derivs, return_all = True, *args)
            except OverflowError:
                f = sys.float_info.max
        else:
            f, derivs_all = objfun(x, known_derivs, return_all = True, *args)
...
def eval_derivative(objfun_derivs, x, component = 0):
    # Evaluate derivatives of the objective function
    f_deriv = objfun_derivs(x,component)
